  ...
  <div class="reservation-container">
    ...
    <div v-if="reservationData && reservationData.length > 0">
      ...
      >
        ...
        <!-- 当状态为预约待确认时显示操作按钮 -->
        <div class="card-actions" v-if="item.status === '预约待确认'">
          <el-button type="primary" @click="confirmRes(item.id)">确认</el-button>
          <el-button type="danger" @click="cancelRes(item.id)">取消</el-button>
        </div>
        ...
        <div
          class="card-actions"
          v-if="item.status === '预约已取消' || item.status === '看房已结束'"
        >
...
const delRes = async (reservation) => {
  try {
    ...
    const updatedReservation = await landlordGetReservation({
      landlord_id: JSON.parse(window.sessionStorage.getItem('user')).id,
    })
...
// 确认预约
const confirmRes = async (reservationId) => {
  try {
    ...
    await confirmReservation({ reservation_id: reservationId })
    const updatedReservation = await landlordGetReservation({
      landlord_id: JSON.parse(window.sessionStorage.getItem('user')).id,
    })
    reservationData.value = updatedReservation
...
onMounted(async () => {
  ...
  reservationData.value = await landlordGetReservation({ landlord_id: userInfo.id })
})
</script>
